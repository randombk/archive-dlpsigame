...
class QueueBuilding {
	 ...
	 * @return bool
	 */
	public static function hasPreReq($playerEnv, $objectEnv, $buildingID, $buildingLevel, $time = TIMESTAMP, $forQueue = false) {
	 ...
	 * @return bool
	 */
	public static function canBuild($playerEnv, $objectEnv, $buildingID, $buildingLevel, $time = TIMESTAMP) {
		if(!self::hasPreReq($playerEnv, $objectEnv, $buildingID, $buildingLevel, $time)) {
			return false;
		}

		if(!$objectEnv->envItems->contains(CalcObject::getBuildingUpgradeCost($playerEnv, $objectEnv, $buildingID, $buildingLevel))) {
			Message::sendNotification(
				$objectEnv->ownerID,
	 ...
	 * @return bool
	 */
	public static function processBuildingQueue($playerEnv, $objectEnv, $time) {
		if(isset($objectEnv->buildingQueue[0])) {
			...
			if($action["operation"] == "Build") {
				...
				$objectEnv->envItems->sum(CalcObject::getBuildingUpgradeCost($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"])->multiply(0.5));
				if($action["buildingID"] == "buildNationalArchives") {
		...
		return self::moveAllUp($playerEnv, $objectEnv, $time);
	}
	 ...
	 * @return bool
	 */
	private static function moveAllUp($playerEnv, $objectEnv, $time) {
		while(sizeof($objectEnv->buildingQueue)) {
			...
			if($action["operation"] == "Build") {
				if(self::canBuild($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"], $time)) {
					$objectEnv->buildingQueue[0]["startTime"] = $time;
					$objectEnv->buildingQueue[0]["endTime"] = $time + CalcObject::getBuildTime($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"]);
					$objectEnv->envItems->sub(CalcObject::getBuildingUpgradeCost($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"]));
					if($action["buildingID"] == "buildNationalArchives") {
				...
				if($action["buildingLevel"] > 0 && $objectEnv->envBuildings->getBuildingLevel($action["buildingID"]) == $action["buildingLevel"]) {
					$objectEnv->buildingQueue[0]["startTime"] = $time;
					$objectEnv->buildingQueue[0]["endTime"] = $time + CalcObject::getBuildTime($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"]) / 10;
					return true;
				} else {
					array_shift($objectEnv->buildingQueue);
					Message::sendNotification(
				...
				if($action["buildingLevel"] > 0 && $objectEnv->envBuildings->getBuildingLevel($action["buildingID"]) == $action["buildingLevel"]) {
					$objectEnv->buildingQueue[0]["startTime"] = $time;
					$objectEnv->buildingQueue[0]["endTime"] = $time + CalcObject::getBuildTime($playerEnv, $objectEnv, $action["buildingID"], $action["buildingLevel"]) / 4;
					return true;
				} else {
					array_shift($objectEnv->buildingQueue);
					Message::sendNotification(
	 ...
	 * @return bool|string
	 */
	public static function appendToBuildingQueue($playerEnv, $objectEnv, $command, $buildingID, $buildingLevel) {
		...
		return self::moveAllUp($playerEnv, $objectEnv, TIMESTAMP) ? false : "An error has occurred. Check your notifications for details";
	}
	 ...
	 * @return bool
	 */
	public static function removeFromBuildingQueue($playerEnv, $objectEnv, $queueItemID) {
			...
			if($objectEnv->buildingQueue[$i]["id"] == $queueItemID) {
				if($i == 0) {
					if($objectEnv->buildingQueue[$i]["operation"] == "Build") {
						$objectEnv->envItems->sum(CalcObject::getBuildingUpgradeCost($playerEnv, $objectEnv, $objectEnv->buildingQueue[0]["buildingID"], $objectEnv->buildingQueue[0]["buildingLevel"])->multiply(0.5));
				...
				self::moveAllUp($playerEnv, $objectEnv, TIMESTAMP);
				return false;
			}
		}
		return false;
